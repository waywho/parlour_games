...
RSpec.describe Game, type: :model do
		...
		describe "to play" do
			context "ghost" do
				...
				def play_complete_word
					game.current_round[:started] = true
					game.save
					game.play_word << "a"
					game.save
					game.play_word << "b"
	...
	context "with teams" do
		...
		it "should cycle through teams" do
			@ordered_teams.cycle(10) do |team|
				@game.next_turn
	...
	context "without teams" do
		let(:game) { FactoryBot.create(:game, team_mode: false) }
		let(:game_sessions) { FactoryBot.build_list(:game_session, 5, game_id: game.id) }
		...
		it "should cycle through the players" do
			player_ids = game_sessions.map(&:id).sort
			player_ids.cycle(12) do |game_session|
				game.next_turn
				expect(player_ids).to include(game.current_turn[:nominated_player])
			end
		end
	end
end
